...
%*************定义终端信息*****************
Mnum=100;   %终端数目
mcn=30; %终端在服务区边缘时，接入点接收到的信号功率（dBm）
...
distance=sqrt(sum(((ones(Mnum,1)*bxy).'-mxy).^2));  %终端距离接入点距离
mrnd=randn(1,Mnum);  %每个终端阴影衰落

%*****************仿真参数****************
G=[0.1:0.1:1,1.2:0.2:2];    %理论业务量
for indx=1:length(G)
    ...
    while 1
        ...
        idx=find(mtime==now_time&Mstate==COLLISION);    %数据包传输失败的ID
        if  length(idx)>0
            Mstate(idx)=STANDBY;
            mtime(idx)=now_time-Rint*log(1-rand(1,length(idx)));    %重新发送时刻
        end
        
        idx=find(mtime==now_time);  %开始传输数据包的终端
        if length(idx)>0
            Mstate(idx)=TRANSMIT;
        ...
        Traffic(indx)=Tplen/Srate/now_time;     %统计实际产生的业务量
        S(indx)=Splen/Srate/now_time;   %统计吞吐量
        Delay(indx)=Wtime/TOTAL*Srate/Plen; %统计平均时延
